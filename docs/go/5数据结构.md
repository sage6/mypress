 ...
 * @LastEditTime: 2026-08-27 10:45:05
 * @Description:
-->
...
查找的内层就是5.4.1描述的并行匹配。matchH2是其核心，amd64上由编译器换成SIMD内建函数，其余平台走SWAR软件实现：

```go
// internal/runtime/group.go： H2并行匹配的可移植实现
const (
...
查找循环据此展开：定位起始组，matchH2拿到的候选掩码，逐位核对真实键：若组内有空槽，则探测终止，返回未命中，否则next()到下一组。删除则要小心：从一个已经满的组里面删除键，不能直接标空，否则
探测序列会在此断开，让后续本该被找到的键变成找不到。此时该槽改为ctrlDelete墓碑，探测会越过它继续；只有当组内尚有空槽的时候，删除才可以直接标空。墓碑平时只增不减，统一在扩容重排清除，以免就地
清理打乱正在进行的遍历。
...
map的两条常被问及的语言规则，其实都是上述实现的直接后果。

`&m[k]`不可取址。Go规范禁止对map元素取地址。原因在实现里面一目了然：无论旧版的搬迁还是新版本的扩容与分裂，元素都会在内存中被移动，先前取得的指针随即悬空。语言索性在编译期间就禁止取址，把一个本会在运行时酿成内存安全事故操作，挡在了门外。需要修改个struct类型的元素，只能整体读出，改完在整体写回。

并发读写直接致命。对同一个map并发读写或者写写，不是数据竞争那样结果未定义，而是运行时主动`fatal("concurrent map ...")`终止进程。检测靠Map.writing这个标志；每个写操作进入时把它异或翻转
（writing ^= 1），完成时在翻转回去。

```go
// internal/runtime/maps/map.go：写操作的并发检测
func (m *Map) PutSlot(...) unsafe.Pointer {
    ...
    // ...真正插入...
    if m.writing == 0 {
        fatal("concurrent map wirtes") // 出门发现标志位被别人动过
    }
    m.writing ^= 1 // 翻回
}
```

读操作则只检查，不修改这个标志：进门若发现 `writing != 0`，说明有写者在场，立即终止。用异或而非简单置1，是一个刻意的概率设计：若两个writer同时闯入，两次翻转可能让标志回到0，使出门检查
一方察觉异常的概率更高。这是一种尽力而为的检测，不加锁，不保证必然抓到每一次竞争，只在几乎零成本的前提下，把绝大多数误用尽早暴漏出清晰的奔溃，而非埋成一个日后难查的诡异bug。要在多goroutine
间共享map，仍须由使用者`sync.Mutex`或者`sync.RWMutex`自行保护，或者改用`sync.Map`

### 5.4.4 别家散列表

把Go的两代设计放进更大的图景，能看清哪些是共识，哪些是取舍。
| 语言 / 库 | 设计 | 碰撞处理 | 要点 |
| ---------------------------- | --------------------------------- | -------------------- | -------------------------------------------------------------------- |
| Go ≥ 1.24 | Swiss Table + 可扩展哈希目录 | 开放定址，二次探测 | 控制字并行探测；目录分裂实现渐进扩容 |
| Rust `HashMap` | hashbrown（Swiss Table 移植） | 开放定址，SIMD组探测 | 标准库直接采用 abseil 设计，默认 SipHash 抗洪水 |
| C++ `absl::flat_hash_map` | Swiss Table 原版 | 开放定址，SIMD组探测 | 键值内联连续存储，缓存友好 |
...
值得点出的对照有两处。其一，`std::unordered_map`是标准把实现钉死的反面教材：C++标准要求桶接口与元素引用在rehash间稳定，这等于强制链地址法，使它无法享受Swiss Table的缓存收益，这也是
abseil另起`flat_hash_map`的原因。Go没有这层包袱，map的内存布局从不进入语言契约，于是能在1.24整体换血而不破坏任何用户代码。其二，Rust标准库直接吸纳hashbrown，与Go各自独立地走向统一个
abseil设计，是Swiss Table成为现代开放定址的有力旁证。

性能从不白来。Swiss Table用控制字的额外空间，SIMD/SWAR的实现复杂度，可扩展哈希的目录间接，换来了高填装因子下的仍连续，仍可并行探测的查找。Go把这笔复杂度连同渐进扩容。GC扫描并发检测一并咽下，只为给上层一流朴素的`m[k]`。这正是运行时存在的意义：把复杂藏在用户看不见的的地方。
